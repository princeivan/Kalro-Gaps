...
const Details = () => {
  ...
  const [error, setError] = useState(null)
    ...
    const { title, description,image, site_preparation, planting, fertilizer_application, pest_and_disease_control, harvesting, yield_information, images } = gap;
  return (
    <>
            ...
            {images && images.length > 0 ? (
              images.map((img, index) => (
                <div key={index} className="image-item">
                  <img src={img.image} alt={img.alt_text || `Gap image ${index + 1}`} />
                </div>
              ))
            ) : (
      ...
      <ul>
          {
              gaps.map(({id,title}, index)=> 
                  <li key={index}><NavLink to={`/gaps-detail/${id}/`}>{title}</NavLink></li>
              )
          }
      </ul>
    </div>
